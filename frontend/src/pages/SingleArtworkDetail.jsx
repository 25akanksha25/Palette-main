...
import { getSingleArtworkById } from "../store/artwork/artworkSlice"; // Redux action
import { toast } from "react-toastify"; // For error handling
import Cookies from "js-cookie"; // Importing js-cookie for accessing cookies
...
const SingleArtworkDetail = () => {
  ...
  const [paid, setPaid] = useState(false); // To track payment status

  useEffect(() => {
    dispatch(getSingleArtworkById(id)); // Fetch the artwork
    
    // Check if the artwork has been paid for already (stored in localStorage)
    const paidStatus = JSON.parse(localStorage.getItem("paidProducts")) || {};
    if (paidStatus[artwork?._id]) {
      setPaid(true);
    }
  }, [dispatch, id, artwork?._id]);
    ...
    <div className="p-6 bg-white">
      ...
      <div className="flex flex-col sm:flex-row items-center bg-gray-100 p-8 rounded-lg shadow-lg">
        ...
        <div className="w-full sm:w-1/2">
          ...
          {/* Purchase Button */}
          {user ? (
            user._id !== artwork?.seller?._id &&
            user.userType !== "admin" &&
            user.userType !== "seller" &&
            artwork.status === "available" ? (
              <button
                onClick={handlePayment}
                className={"mt-6 bg-black text-white py-2 px-6 rounded-lg text-lg hover:bg-pink-500 w-full ${paid ? 'cursor-not-allowed bg-gray-400' : ''}"}
                disabled={paid} // Disable if already paid
              >
                {paid ? "Purchased" : "Purchase"}
              </button>
            ) : artwork.status === "paid" || artwork.status === "sold" ? (
              <p className="mt-6 text-red-500 text-lg font-semibold">
                This artwork is no longer available.
